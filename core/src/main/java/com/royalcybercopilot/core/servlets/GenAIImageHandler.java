...
public class GenAIImageHandler extends SlingAllMethodsServlet {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(
      GenAIImageHandler.class);

  @Reference
  transient AiTokenConfigurationsImpl aiTokenConfigurations;
    ...
    try {
      ...
      switch (inputData.getOperationname()) {
        case "Generate":
          ...
          LOGGER.info("img response {}", imageResponse);
          LOGGER.info("img response data {}", imageResponse.getData());
          if (imageResponse.getError() != null) {
            response.setStatus(500);
            response.getWriter().write(imageResponse.getError().getMessage());
          } else {
            response.getWriter().write(gson.toJson(imageResponse.getData()));
          }
          break;
        case "SaveImage":
          // Save the image to the DAM
          String url = saveImageToDAM(request.getResourceResolver(), inputData);
      ...
      response
          .getWriter()
          .write("Failed to Complete the Request. Please try again later");
    }
  }
  ...
  private ImageResponse sendPrompt(InputData inputData) throws IOException {
    ...
    String requestBody = MAPPER.writeValueAsString(
        ImageRequest
            .builder()
            .prompt(inputData.getPrompt())
            .n(3)
            .size(inputData.getSizevalue())
            .build());
  ...
  private String saveImageToDAM(
    ...
    try {
      ...
      String imagePath = inputData.getDamfolderpath() +
          "/" +
          inputData.getImagename() +
          "." +
          fileExt;
      LOGGER.info(fileExt);
      resourceResolver
          .adaptTo(AssetManager.class)
          .createAsset(imagePath, is, mimeType, true);
